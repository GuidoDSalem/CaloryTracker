...
class CalculateMealNutrients(
    ...
    operator fun invoke(trackedFoods: List<TrackedFood>): Result{
        ...
        val caloriesGoal = dailyCaloryRequirements(userInfo)
    ...
    private fun bmr(userInfo: UserInfo): Int{
        return when(userInfo.gender){
            is Gender.Male -> {
                (66.47f + 13.75f * userInfo.weight + 5f * userInfo.height - 4.67f * userInfo.age).roundToInt()
            }
            is Gender.Female -> {
                (65.09f + .56f * userInfo.weight + 1.84f * userInfo.height - 4.67f * userInfo.age).roundToInt()
            }
        }
    }

    private fun dailyCaloryRequirements(userInfo: UserInfo): Int{
